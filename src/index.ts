...
async function requireAdmin(req: Request & { userId?: string }, res: Response, next: NextFunction) {
  ...
  try {
    const [rows] = await pool.query('SELECT role FROM profiles WHERE id = $1 LIMIT 1', [req.userId]);
  ...
  try {
    ...
    await pool.query(
      `INSERT INTO profiles (id, email, full_name, phone, role, password_hash)
       VALUES (gen_random_uuid(), $1, $2, $3, 'client', $4)`,
      [email, full_name, phone ?? null, passwordHash]
    );

    const [rows] = await pool.query(
      'SELECT id, email, full_name, phone, role, created_at, updated_at FROM profiles WHERE email = $1 LIMIT 1',
      [email]
    );
    ...
    if (profile?.id) {
      try {
        await pool.query(
          'UPDATE appointments SET client_id = $1 WHERE client_id IS NULL AND contact_email = $2',
          [profile.id, email]
        );
      } catch (linkError) {
...
app.post('/auth/login', async (req: Request, res: Response) => {
  ...
  try {
    const [rows] = await pool.query(
      'SELECT id, email, full_name, phone, role, created_at, updated_at, password_hash FROM profiles WHERE email = $1 LIMIT 1',
      [email]
    );
...
app.get('/auth/me', authMiddleware, async (req: Request & { userId?: string }, res: Response) => {
  ...
  try {
    const [rows] = await pool.query(
      'SELECT id, email, full_name, phone, role, created_at, updated_at FROM profiles WHERE id = $1 LIMIT 1',
      [req.userId]
    );
...
app.post('/auth/reset-password', async (req: Request, res: Response) => {
  ...
  try {
    const [rows] = await pool.query(
      'SELECT pr.user_id FROM password_resets pr WHERE pr.token = $1 AND pr.used = false AND pr.expires_at > NOW() LIMIT 1',
      [token]
    );
...
app.patch('/profile', authMiddleware, async (req: Request & { userId?: string }, res: Response) => {
  ...
  try {
    await pool.query(
      'UPDATE profiles SET full_name = COALESCE($1, full_name), phone = COALESCE($2, phone), updated_at = NOW() WHERE id = $3',
...
app.get('/services', async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(
      'SELECT id, name, description, duration_minutes, price, active, created_at FROM services WHERE active = true ORDER BY name ASC'
    );
    res.json(rows);
  } catch (error) {
...
app.post('/appointments', async (req: Request, res: Response) => {
  ...
  try {
    // Verificar se a data ou o horário estão bloqueados na tabela schedule_blocks
    const [blocks] = await pool.query(
      'SELECT id FROM schedule_blocks WHERE block_date = $1 AND (block_time IS NULL OR block_time = $2) LIMIT 1',
      [appointment_date, appointment_time]
    );

    if ((blocks as any[]).length > 0) {
      return res.status(409).json({ error: 'Data ou horário indisponível para agendamento.' });
    }

    const [existing] = await pool.query(
      'SELECT id FROM appointments WHERE appointment_date = $1 AND appointment_time = $2 LIMIT 1',
      [appointment_date, appointment_time]
    );
    ...
    await pool.query(
      'INSERT INTO appointments (id, client_id, service_id, appointment_date, appointment_time, notes, status, contact_name, contact_phone) VALUES (gen_random_uuid(), NULL, $1, $2, $3, $4, $5, $6, $7)',
...
app.get('/appointments/unavailable', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if ((dayBlockRows as any[]).length > 0) {
    ...
    const [rows] = await pool.query(
      ...
      (
        ...
        WHERE block_date = $2 AND block_time IS NOT NULL
      )`,
      [date, date]
    );

    const times = (rows as any[]).map((r) => r.time);
    res.json(times);
  } catch (error) {
...
// Admin - schedule blocks (listar todos)
app.get('/admin/schedule-blocks', authMiddleware, requireAdmin, async (_req: Request, res: Response) => {
  try {
    const [rows] = await pool.query(
      `SELECT id, block_date, block_time, reason, created_by, created_at
       FROM schedule_blocks
       ORDER BY block_date ASC, block_time ASC`
    );
    res.json(rows);
  } catch (error) {
    console.error('Error fetching schedule blocks', error);
    res.status(500).json({ error: 'Erro ao buscar bloqueios de agenda' });
  }
});

// Admin - criar novo bloqueio de agenda
app.post('/admin/schedule-blocks', authMiddleware, requireAdmin, async (req: Request & { userId?: string }, res: Response) => {
  const { block_date, block_time, reason } = req.body as {
    block_date?: string;
    block_time?: string | null;
    reason?: string | null;
  };

  const normalizedBlockTime = block_time === '' || block_time === undefined ? null : block_time;
  const normalizedReason = reason === '' || reason === undefined ? null : reason;

  if (!block_date) {
    return res.status(400).json({ error: 'block_date é obrigatório' });
  }

  try {
    // Evitar bloqueio duplicado para o mesmo dia/horário
    const [existing] = await pool.query(
      'SELECT id FROM schedule_blocks WHERE block_date = $1 AND ((block_time IS NULL AND $2::time IS NULL) OR block_time = $2::time) LIMIT 1',
      [block_date, normalizedBlockTime]
    );

    if ((existing as any[]).length > 0) {
      return res.status(409).json({ error: 'Já existe um bloqueio para esta data/horário.' });
    }

    await pool.query(
      'INSERT INTO schedule_blocks (id, block_date, block_time, reason, created_by) VALUES (gen_random_uuid(), $1, $2, $3, $4)',
      [block_date, normalizedBlockTime, normalizedReason, req.userId ?? null]
    );
    ...
    console.error('Error creating schedule block', {
...
// Admin - remover bloqueio de agenda
app.delete('/admin/schedule-blocks/:id', authMiddleware, requireAdmin, async (req: Request, res: Response) => {
  const { id } = req.params;
  ...
  try {
    ...
    res.status(500).json({ error: 'Erro ao excluir bloqueio de agenda' });
  }
});
