...
app.post('/appointments', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [blocks] = await pool.query(
      'SELECT id FROM schedule_blocks WHERE block_date = ? AND (block_time IS NULL OR block_time = ?) LIMIT 1',
  ...
  try {
    const [rows] = await pool.query(
      ...
      (
        ...
        WHERE block_date = ? AND block_time IS NOT NULL
      )`,
      [date, date]
    );
...
app.post('/admin/schedule-blocks', authMiddleware, requireAdmin, async (req: Request & { userId?: string }, res: Response) => {
  ...
  try {
    ...
    const [existing] = await pool.query(
      'SELECT id FROM schedule_blocks WHERE block_date = ? AND ((block_time IS NULL AND ? IS NULL) OR block_time = ?) LIMIT 1',
      [block_date, block_time, block_time]
    );
    ...
    await pool.query(
      'INSERT INTO schedule_blocks (id, block_date, block_time, reason, created_by) VALUES (UUID(), ?, ?, ?, ?)',
      [block_date, block_time ?? null, reason ?? null, req.userId ?? null]
    );

    res.status(201).json({ success: true });
  } catch (error) {
    console.error('Error creating schedule block', error);
    res.status(500).json({ error: 'Erro ao criar bloqueio de agenda' });
  }
});
...
app.delete('/admin/schedule-blocks/:id', authMiddleware, requireAdmin, async (req: Request, res: Response) => {
  ...
  try {
    const [result] = await pool.query('DELETE FROM schedule_blocks WHERE id = ?', [id]);
